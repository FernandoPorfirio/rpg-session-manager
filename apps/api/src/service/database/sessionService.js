...
const create = async ({ name, maxLevel, gameMasterId, lore, sessionStatusId = 1 }) => {
...
const update = async ({ id, name, maxLevel, sessionStatusId, lore, startedAt, finishedAt }) => {
  const updateData = {
    updated_at: db.fn.now()
  };
  ...
  if (sessionStatusId !== undefined) updateData.session_status_id = sessionStatusId;
...
const getByGameMasterId = async (gameMasterId) => {
  const result = await db("session")
    ...
    .where({ "session.game_master_id": gameMasterId, "session.is_deleted": false });
